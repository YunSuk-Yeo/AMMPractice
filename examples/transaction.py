...
#:!:>section_3
    ...
    def account_modules(self, account_address: str) -> Optional[Dict[str, Any]]:
        response = requests.get(f"{self.url}/accounts/{account_address}/modules")
        if response.status_code == 404:
            return None
        assert response.status_code == 200, response.text
        return response.json()

    def account_module(self, account_address: str, module_name: str) -> Optional[Dict[str, Any]]:
        response = requests.get(f"{self.url}/accounts/{account_address}/module/{module_name}")
        if response.status_code == 404:
            return None
        assert response.status_code == 200, response.text
        return response.json()
